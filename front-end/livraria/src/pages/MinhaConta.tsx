...
const MinhaConta = () => {
  const [form, setForm] = useState({
    fullName: "Novo Nome", // Nome atual do usuário
    password: "",
    confirmPassword: "",
  });
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirmDelete) {
      try {
        // Simula a chamada para o backend
        const response = await fetch("https://seu-backend.com/api/delete-account", {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ userId: "id-do-usuario" }), // Substitua pelo ID do usuário real
        });

        if (response.ok) {
          setSuccess("Conta deletada com sucesso!");
          console.log("Conta deletada");
          // Redirecionar ou limpar o estado após a exclusão
        } else {
